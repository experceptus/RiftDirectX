...
int WINAPI WinMain(HINSTANCE hinst, HINSTANCE, LPSTR, int)
{
	...
	while (DIRECTX.HandleMessages())
	{
		...
		if (DIRECTX.Key[VK_LEFT])  mainCam.Rot = Matrix4f::RotationY(Yaw += 0.01f);
		if (DIRECTX.Key[VK_RIGHT]) mainCam.Rot = Matrix4f::RotationY(Yaw -= 0.01f);

		// Keyboard inputs to adjust player position
		if (DIRECTX.Key['W'] || DIRECTX.Key[VK_UP])   mainCam.Pos += mainCam.Rot.Transform(Vector3f(0, 0, -0.025f));
		if (DIRECTX.Key['S'] || DIRECTX.Key[VK_DOWN]) mainCam.Pos += mainCam.Rot.Transform(Vector3f(0, 0, +0.025f));
		if (DIRECTX.Key['D'])                         mainCam.Pos += mainCam.Rot.Transform(Vector3f(+0.025f, 0, 0));
		if (DIRECTX.Key['A'])                         mainCam.Pos += mainCam.Rot.Transform(Vector3f(-0.025f, 0, 0));
		if (DIRECTX.Key['Q']) mainCam.Pos += Vector3f(0, +0.025f, 0);
		if (DIRECTX.Key['E'])  mainCam.Pos += Vector3f(0, -0.025f, 0);
		...
		if (isVisible)
		{
			...
			for (int eye = 0; eye < 2; eye++)
			{
				...
				newBox.Render2(proj*view, 1, 1, 1, 1, true);
			}
		}
