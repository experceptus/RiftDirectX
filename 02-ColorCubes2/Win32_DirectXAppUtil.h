...
struct DirectX11
{
    ...
    bool InitWindowAndDevice(HINSTANCE hinst, OVR::Recti vp, LPCWSTR title = nullptr)
    {
		...
		// Use DXGI to create a Device and DeviceContext
		...
		// Why does DirectX DXGI report two adapters for one video card?
		for (UINT i = 0;
			DXGIFactory->EnumAdapters(i, &Adapter) != DXGI_ERROR_NOT_FOUND;
			++i)
		{
			// Look at vAdapters in the debugger
...
struct Scene  
{
    ...
    Scene(bool removeFurniture) : num_models(0) 
    {
        TriangleSet cube;
		...
		// Thin staff in the (almost) center of the room
		TriangleSet redStaff;
		redStaff.AddSolidColorBox(0.0f, 0.0f, 0.0f, 0.05f, 4.0f, 0.05f, 0xffff0000);
		// for those who want it in the exact center of the room
		//redStaff.AddSolidColorBox(-0.025f, 0.0f, -0.025f, 0.025f, 4.0f, 0.025f, 0xffff0000);
		Add(new Model(&redStaff, Vector3f(0, 0, 0), new Material(new Texture(false, Sizei(256, 256), Texture::AUTO_FLOOR))));

		// Thin staff at 0,0,20, the end of the room
		TriangleSet yellowStaff;
		yellowStaff.AddSolidColorBox(0.0f, 0.0f, 0.0f, 0.05f, 4.0f, 0.05f, 0xffffff00);
		Add(new Model(&yellowStaff, Vector3f(0, 0, 20), new Material(new Texture(false, Sizei(256, 256), Texture::AUTO_FLOOR))));
